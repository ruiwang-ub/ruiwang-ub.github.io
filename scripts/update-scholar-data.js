...
async function fetchScholarData() {
  try {
    ...
    // Write fetched data to JSON file for Home.js to consume
    fs.writeFileSync(outputPath, JSON.stringify({
      metrics,
      articles: data.articles || []
    }, null, 2));
    console.log(`📥 Scholar data cached to ${outputPath}`);
    ...
    // Disabled updateHomeComponent(metrics) call since Home.js reads from JSON now
...
function updateHomeComponent(metrics) {
  ...
  const articlesEntries = metrics.articles
    .map((a) => `            { title: "${a.title.replace(/"/g, '\\"')}" }`)
    .join(",\n");
